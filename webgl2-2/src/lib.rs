...
pub fn start() -> Result<(), JsValue> {
    ...
    //正方形
    let white_cube = cube::create_cube(0., 5., 1., 1., 1., 1.);
    ...
    let g = frame.clone();
    let mut i = 0.;
    *g.borrow_mut() = Some(Closure::wrap(Box::new(move ||{if i >= 360. {
            i = 0.;
        } 
        i += 1.;
    ...
    m_matrix
        .set_identity()
        //.ArbitraryAxisRotation([0.,0.,1.],rad);
        .Quaternionrotation([0.,0.,1.],rad);
